...
from app.model.video import VideoType, FromType, NationalityType, Videos, VideoSub, IdentityType, VideoStar
...
class ExternalVideo(View):
    ...
    def get(self, request):
        ...
        data = {}
        data['error'] = error
        cus_videos = Videos.objects.filter(from_to=FromType.custom.value)
        ex_videos = Videos.objects.exclude(from_to=FromType.custom.value)
    ...
    def post(self, request):
        ...
        if video_id:
            path = reverse("video_update", kwargs={'video_id': video_id})
        else:
            path = reverse('external_video')
        ...
        if not video_id:
            ...
            try:
                ...
                return redirect('{}?error={}'.format(path, '修改失败'))
...
class VideoSubView(View):
    ...
    def post(self, request, video_id):
        ...
        url_format = "{}".format(reverse('video_sub', kwargs={'video_id': video_id}))
        video = Videos.objects.get(pk=video_id)
